...
public class SMILEBayesNet extends AbstractValueDAG<String,String> implements MutableBayesNet<String,String> {
    ...
    public double probabilities2( List<? extends String> nodes, ArrayList<String> onePermutation){
        ...
        for (int z=0; z<nodeIDs.length; z++){
        	nodeIDList.add(nodeIDs[z]);
        }
        for (int i=0; i<nodes.size(); i++){
        	String oneNode = nodes.get(i);
        	if (nodeIDList.contains(oneNode)){
        		nodeArray[i] = oneNode;
        		int nodeIndex = net.getNode(oneNode);
        		vars[i] = nodeIndex;
        		String[] nodeOutcomes = net.getOutcomeIds(nodeIndex);
        		ArrayList<String> nodeOutcomeList = new ArrayList<String>();
        		for (int n=0; n<nodeOutcomes.length; n++){
        			nodeOutcomeList.add(nodeOutcomes[n]);
        		}
        		String currentNodeOutcome = onePermutation.get(i);
        		int outcomeIndex = -1; 
        		outcomeIndex = nodeOutcomeList.indexOf(currentNodeOutcome);
        		vals[i] = outcomeIndex;
        		if (outcomeIndex==-1) {
        			new IllegalArgumentException( "Was not able to find handles for "+oneNode+" outcome " + currentNodeOutcome + " !" );
        		}
        		
        	}
        	else {
        		new IllegalArgumentException( "Was not able to find handles for "+oneNode+"!" );
        	}
        }
        
       for (int v=0; v<vars.length; v++){
        	//System.out.println("v=" + v + " "+ vars[v]+","+ vals[v]);
        	//System.out.println(net.getNodeName(vars[v])+" " + ","+ net.getOutcomeIds(vars[v])[vals[v]]);
        }
 
        // Query BN
        double p=1.0;
        try{
            p = probabilities2(new Double("1.0"), vars, vals, 0);
        }catch( WrException e ){
            throw new WrException( "Failed to get P( "+nodes +").", e);
        }
    ...
    /**
      ...
      * @param nodes
      * @param vals
      * @param conditionPoint
      * @return
      */
	 private double probabilities2(double currentProb, final int[] nodes, final int[] vals, final int conditionPoint ){
	        
	        // Base case
	        if( conditionPoint >= nodes.length ) return currentProb;	        
	        // Recursive case
	        // Get last node
	        double single = probabilities2(nodes[conditionPoint], vals[conditionPoint], nodes, vals, conditionPoint );
	        currentProb = currentProb * single;
	        // Multiply probabilities
	        double result = probabilities2(currentProb, nodes, vals, conditionPoint+1 );        
	...
	//        }
	  ...
	  * @return
	  */
	 private double probabilities2( int node, int nodeValueIndex, int[] cNodes, int [] cVals, int conditions ){
	        ...
	        try{
	            ...
	            net.updateBeliefs();
	            double[] result = net.getNodeValue(node);      
	            return result[nodeValueIndex];
	        }catch( SMILEException e ){
    ...
    public SMILEBayesNet( Network net, boolean convertIDs ){
        this.net = net;
...
//                throw new IllegalArgumentException( "Was not able to find handles for all of "+nodes+"!" );
        ...
        double[] pArray;
        try{
            pArray = probabilities( vars, vals, 0);
...
//            System.out.println(pArray.length); 
        ...
        // Map result back
        if (pArray.length!=permutationList.size()){
        	try {
				throw new Exception("number of permutations and the number of probabilities does not match!");
			} catch (Exception e) {
        ...
        for (int k=0; k<pArray.length; k++){
        	result.put(permutationList.get(k), pArray[k]);
        	//System.out.println(permutationList.get(k) + "," + pArray[k]);
        }
        return result;
    }
...
//            System.out.println(nodeArray[k]);
//            for (int p=0; p<valueArray[k].length; p++){
//            	System.out.println(valueArray[k][p]);
//            }         
        }
    ...
    private double[] probabilities( final int[] nodes, final int[] vals, final int conditionPoint ){
        ...
        int blockSize = 1;
        for ( int cp = conditionPoint+1; cp < nodes.length; cp++ )
            blockSize *= net.getOutcomeCount( nodes[cp] );
...
//        }
     ...
     * Return p(node | cNodes[0] = cVals[0],...., cNodes[conditions-1] = cVals[conditions-1] ) 
